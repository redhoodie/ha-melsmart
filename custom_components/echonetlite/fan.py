...
from .pychonet_ext.mappings import (
...
# Home Air Cleaner (0x35), Ceiling Fan (0x3A), Ventilation fan (0x33),
# Air-conditioner ventilation fan / Lossnay (0x34)
_FAN_EOJCC = {0x33, 0x34, 0x35, 0x3A}
...
async def async_setup_entry(hass, config_entry, async_add_devices):
    ...
    for entity in hass.data[DOMAIN][config_entry.entry_id]:
        if entity["instance"]["eojgc"] == 0x01 and (
            entity["instance"]["eojcc"] in _FAN_EOJCC
...
class EchonetFan(EchonetEntity, FanEntity):
    ...
    def __init__(self, coordinator, config) -> None:
        ...
        self._is_ventilation = (
            coordinator._eojgc == 0x01 and coordinator._eojcc in VENTILATION_EOJCC
        )
        ...
        # Ventilation fans expose 0xA0 as named presets AND a 4-speed percentage.
        if self._is_ventilation and self.is_settable(ENL_FANSPEED):
            self._attr_supported_features |= FanEntityFeature.SET_SPEED
    ...
    def percentage(self) -> int | None:
        ...
        if self._is_ventilation:
            return preset_to_percentage(self.coordinator.data.get(ENL_FANSPEED))
    ...
    def preset_modes(self) -> list[str] | None:
        ...
        instance_presets = getattr(self.coordinator._instance, "PRESET_MODES", None)
        if instance_presets:
            return list(instance_presets)
        if self._is_ventilation:
            return list(DEFAULT_LOSSNAY_PRESETS)
    ...
    async def async_set_percentage(self, percentage: int) -> None:
        ...
        if self._is_ventilation:
            ...
            if preset is None:
                await self.async_turn_off()
                return
            await self.coordinator._instance.setFanSpeed(preset)
            return
