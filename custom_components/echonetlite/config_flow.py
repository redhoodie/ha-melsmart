...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        for instance in self._config_entry.data["instances"]:
            ...
            elif instance["eojgc"] == 0x01 and instance["eojcc"] in (
                0x33,
                0x34,
            ):  # Ventilation fan / Lossnay
                if ENL_FANSPEED in instance["setmap"]:
                    option_default = list(LOSSNAY_FAN_SPEED_OPTIONS.keys())
                    if (
                        self._config_entry.options.get(
                            USER_OPTIONS[ENL_FANSPEED]["option"]
                        )
                        is not None
                    ):
                        option_default = self._config_entry.options.get(
                    ...
                    data_schema_structure.update(
                        {
                            vol.Optional(
                                USER_OPTIONS[ENL_FANSPEED]["option"],
                                default=option_default,
                            ): cv.multi_select(LOSSNAY_FAN_SPEED_OPTIONS)
                        }
                    )
