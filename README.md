...
**Fork with Mitsubishi Lossnay (ERV) support.** This repository is based on
[scottyphillips/echonetlite_homeassistant](https://github.com/scottyphillips/echonetlite_homeassistant) v4.0.8
and adds local ECHONET Lite control for ventilation fans — power on/off and
fan speeds **1 / 2 / 3 / 4 / Boost** — targeting Mitsubishi Electric Lossnay
units such as the **VL-500** on a **MAC-578IF** Wi-Fi interface.
...
Ventilation classes `0x0133` and `0x0134` are implemented in this fork (they are
named in pychonet but had no device class or Home Assistant fan entity).
...
| Mitsubishi Electric | Lossnay VL-250 / VL-350 / VL-500 + MAC-578IF   | AirConditionerVentilationFan / VentilationFan | Fan, Sensor, Select | **This fork.** Power, speeds 1–4, Boost. Enable ECHONET on the *Lossnay* adapter, then add its IP. See [LOSSNAY.md](LOSSNAY.md). |
...
This is a **fork**. Add it as a custom HACS repository (the upstream
`scottyphillips/echonetlite_homeassistant` listing will not include Lossnay).

### Via HACS (custom repository)

1. HACS → Integrations → ⋮ → Custom repositories
2. URL: this repository, category **Integration**
3. Download **ECHONETLite Platform**, then restart Home Assistant
...
## Mitsubishi Lossnay (this fork)

Lossnay is a **separate Wi-Fi device** from the ducted heat pump. In the
Mitsubishi AU/NZ app it appears as its own unit (model VL-500, adapter type
`mac578`) with Power, Lossnay / Bypass / Auto, Fan Speed 1–4, and Boost.

1. In the official app, open the **Lossnay** unit (not the heat pump) →
   **Settings** → enable **ECHONET Lite** (MAC-568 / 578 / 588 only).
2. Give the Lossnay adapter a DHCP reservation.
3. In Home Assistant add **ECHONET Lite** and enter the **Lossnay adapter IP**.
4. You should get a **Fan** entity with on/off, presets `1`, `2`, `3`, `4`,
   `boost`, and a 4-step speed slider.

If discovery finds nothing, or only a Home Air Conditioner (`0x0130`) object,
the adapter is not exposing a ventilation class. That is a firmware limitation,
not a missing HA entity. Full notes, EPC tables, and fallbacks are in
[LOSSNAY.md](LOSSNAY.md).
...
Lossnay speed/mode mappings are covered without Home Assistant:
